...
docker exec -it ${CONTAINER} mysql -u 'root' -p"${MYSQL_ROOT_PASS}" -e " \
...
docker exec -it ${CONTAINER} bin/bash -c "cat /create-tables.sql | sed 's/#MYSQL_DB#/${MYSQL_DB}/' | mysql -u"${MYSQL_USER}" -p"${MYSQL_PASS}
